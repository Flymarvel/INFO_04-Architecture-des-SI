...
public class ParcoursUnitTest
{
    ...
    public async Task AddEtudiantDansParcoursUseCase()
    {
        ...
        // Mock pour FindByConditionAsync : retourner différentes listes selon la condition
        // Première appel : cherche juste l'étudiant par ID -> retourner l'étudiant
        // Deuxième appel : vérifie si l'étudiant est déjà inscrit -> retourner liste vide
        mockEtudiant
            .SetupSequence(repo => repo.FindByConditionAsync(It.IsAny<Expression<Func<Etudiant, bool>>>()))
            .ReturnsAsync(new List<Etudiant> { new Etudiant { Id = 1, NumEtud = "1" } })  // Premier appel : étudiant trouvé
            .ReturnsAsync(new List<Etudiant>());  // Deuxième appel : pas d'inscription existante

        List<Parcours> parcoursInitial = new List<Parcours>();
        parcoursInitial.Add(new Parcours { Id = 3, NomParcours = "Ue 3", AnneeFormation = 1 });
        ...
        if (parcoursFinal.Inscrits != null)
        {
    ...
    public async Task AddUeDansParcoursUseCase()
    {
        ...
        if (parcoursAModifier.UesEnseignees != null)
        {
